"""Controlled source-system adapter for accepted dependency Deliveries."""
...
def apply_delivery_to_source_copy(
    ...
    *,
    ...
    """Return a validated source copy with one accepted dependency Delivery applied."""
    ...
    if delivery.get("user_attitude") != "accepted":
        raise ScheduleDeliveryApplicationError("candidate has not been accepted")
    ...
    for operation in delivery["effective_patch"]["operations"]:
        if operation["operation"] == "remove_dependency":
            ...
            if dependencies.get(dependency_id) != operation["expected_before"]:
                raise ScheduleDeliveryApplicationError(
                    f"remove precondition failed for dependency {dependency_id}"
                )
            ...
            if operation["expected_before"] is not None or dependency_id in dependencies:
                raise ScheduleDeliveryApplicationError(
                    f"add precondition failed for dependency {dependency_id}"
                )
            dependencies[dependency_id] = dependency
        else:
            raise ScheduleDeliveryApplicationError(
                f"unsupported delivery operation {operation['operation']}"
            )
...
def _update_source_validation(
    ...
    validation["capabilities"] = {
        name: capability.model_dump(mode="json") for name, capability in capabilities.items()
    }
    ...
    network_refs = {
        ...
        "SUMMARY_TASK_DEPENDENCIES": sorted(
            ...
            if item["predecessor_task_id"] in summary_ids
            or item["successor_task_id"] in summary_ids
        ),
    }
    ...
    network_quality["summary_task_dependency_ids"] = network_refs[
        "SUMMARY_TASK_DEPENDENCIES"
    ]

    assessment = validation["source_vs_conversion_assessment"]
    assessment["source_mpp_findings"] = _project_network_findings(
        assessment["source_mpp_findings"], network_refs
    )
    ...
    removed_ids = {
        item["dependency_id"] for item in delivery["effective_patch"]["removed_dependencies"]
    }
