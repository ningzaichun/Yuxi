...
- 新增基础设施与本地调试程序拆分部署方案：保留原一体化 Compose，新增服务器端 PostgreSQL、Redis、MinIO、Milvus/Etcd、Neo4j 编排，以及本地 API、Worker、Web、Sandbox 热重载编排；提供独立环境模板和 PowerShell 初始化、启停、验证脚本，强制基础设施密钥、显式忽略实际配置文件，并补充网络隔离、数据迁移、备份、验收与回滚计划。新增本地启动与二次开发手册，覆盖一体化/拆分模式选择、首次初始化、热重载与镜像重建边界、容器内测试、模块落位、数据保护和常见故障排查。
- 完成 K-AI 第一阶段可视化品牌改造：默认品牌配置调整为“量程科技 · K-AI”，产品定位更新为建筑行业智能知识库与智能体平台；浏览器标题、favicon、登录页、首页、主侧边栏、设置与用户菜单、CLI 授权页及知识库类型标签统一使用新品牌展示，并移除上游 GitHub Star、仓库和帮助文档等产品界面入口。内部 `yuxi` 包名、环境变量、接口协议、缓存键、Docker 与数据库结构保持不变；正式品牌素材、官网和帮助地址待提供后再替换临时标识。
