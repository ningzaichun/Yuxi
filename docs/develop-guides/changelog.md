...
- 新增排期外部 JSON 导入基础层：定义带严格身份字段的 `ScheduleImportEnvelope` 和允许来源扩展字段的 `microsoft_project_interchange_mock_v1.1` 边界契约，缺少核心字段、已定义字段类型错误、未知引用和父子层级环仍会拒绝；新增按 `schema_version` 路由的 Adapter Registry，以及纯 Python、跨平台的水泵站 Microsoft Project interchange 适配器，在不读取 MPP、不依赖 Windows 的前提下生成严格 `canonical_schedule_v2.2`。来源原文与未知字段保持不变，来源自报 Statistics、Capabilities 和 Validation 不参与内部结论，Yuxi 重新计算 21 个任务、19 条依赖、开放起止点和 Capability；来源缺失的 Baseline、Source Calculation、项目当前日期及资源分配通过规范化报告显式标记为未参与审查或计算，4 个零工期里程碑使 CPM 明确返回 `MILESTONE_UNSUPPORTED`。同一来源重复转换及仅增加未知字段时 Canonical 哈希保持一致；本轮仅完成契约与适配器，不新增 API、数据库或对象存储行为。136 项 Schedule 相关单元测试、7 项真实 HTTP/PostgreSQL/MinIO 集成测试和 7 套 Microsoft Project 黄金门禁均通过。
