...
- 支持通过独立本机 HTTP 入口接入第三方 Blender MCP，补充启动与智能体配置说明；修复 MCP 连接测试在零工具时误报成功的问题。
...
- 完成 Y3 大型 Suite 严格协议：新增 `schedule_engine_large_suite_adapter_v2` 强类型适配契约，在不修改冻结 `schema_version` 和 Suite 文件的前提下显式接入 `test_profile`、WBS 与 Outline Level，未知顶层或任务字段直接拒绝；Runner 新增逐案例 `gate.json` 和确定性 `suite_run_summary.json`，区分 Passed、Unsupported、Failed 与 UI 导出资格。逐条复算确认大型 Oracle 使用 Assignment 对资源冲突；真实容量复核进一步补上“三项及以上合计超配但任意两项不超配”的漏报，保留两两证据并仅在两两证据不能揭示超配时补充累计时间片，Resource Profile 升级到 v3，含 inactive 的 Profile 同步升级到 v17。L06 的 500 活动、500 Assignment、20 资源、100 条冲突、500 条两位小数成本及全部日期仍与 Oracle 严格一致；L02 的 325 条两两冲突和成本可重建，但 Oracle 遗漏 135 个累计超配时间片，另有 4 个负 FF Lag 里程碑在午休/日终反向位移边界冲突，因此以两个精确原因继续 Unsupported，来源和冻结 Oracle 均不改写。当前汇总为 2 Passed、4 Unsupported、0 Failed、1 UI Export Eligible；L06 经页面自身校验导入后，从小型快照切换到 673 行表格和 2,615 个甘特元素用时 1.32 秒，页面约 9,958 个 DOM 节点且控制台无 Error/Warning，测试 Snapshot 与对象已清理。该结果只关闭 500 活动 Suite UI 基线；页面仍为全量 DOM，更大或真实业务规模继续保持生产门禁。冻结 Suite 仍可由仓库内入口逐字节重建，不修改来源或 Oracle，生产结论保持 `PRODUCTION_NO_GO`。

- 完成 Y2 小型 Suite Reference 日期修复：`SuiteCaseExecution` 显式暴露 Engine Result，C01-C06 仅在 Actual 与冻结 Oracle 结果契约完全一致后投影 early/late、浮时、关键标识和项目日期，并以 `YUXI_TEST_SUITE_ENGINE_REFERENCE`、`source_dates_preserved=false` 明确标记为 Yuxi 测试参考计算；C08 保留 Microsoft Project Observation。投影后由正式 Audit Context 重算 statistics、Capability、Validation 和日期违规，C04 的预期硬约束冲突得到保留；C07、Unsupported、缺失计算结果和 Oracle Mismatch 均在落盘前拒绝。两次 7 文件导出的逐文件 SHA-256 完全一致，C01 实际 `/snapshots → Audit → Candidate` 通过且无统计漂移；Schedule 单元回归、Ruff、前端 Node/ESLint/生产构建通过。Chrome 完整刷新验证 Suite 标签与导入弹窗正常，实际本地文件选择受浏览器扩展文件 URL 权限限制，未误报为上传成功，等价导入链路由真实 API 集成覆盖。阶段更新为 `Y2 PASSED / Y3 READY`，生产结论保持 `PRODUCTION_NO_GO`。

- 完成 Y1 API/UI 溯源与 Candidate 闭环：排期页面新增 MPP/测试 Suite 来源类型、提取方式、来源 JSON SHA-256、Canonical SHA-256 和 Adapter 展示，MPP 计划日期、Suite Reference 测试参考日期和 Yuxi 候选计算日期使用不同标签；Candidate 抽屉显示基础 Source Hash，并继续明确候选不覆盖 Source。重算与目标优化按钮统一服从 `cpm_recalculation` Capability，阻断时禁用并就地展示稳定原因。新增来源标签与 Capability 操作状态前端回归，定向 ESLint、Web 生产构建、正式 Import→Audit→Candidate 集成和 Schedule Agent E2E 通过；浏览器实测 MPP 双哈希/提取方式、Suite 标签、Candidate 溯源及阻断按钮均符合预期，控制台无新增错误。阶段更新为 `Y1 PASSED / Y2 READY`，生产结论保持 `PRODUCTION_NO_GO`。

- 完成 MPP Bridge → Yuxi Y0 全部门禁：发布由正式 Pydantic 模型生成并同步进入 Bridge 分发包的 `microsoft_project_interchange_v1_1.schema.json`，新增严格 Projection Report 和 `Project-MppBridgeSnapshotToYuxi.ps1`。Projector 在落盘前验证 Snapshot Schema、Semantic Hash、unsupported 与 roundtrip eligibility，拒绝覆盖现有输出，并对 v1.1 无法表达的非空 Status Date fail-closed；字段处置逐类记录保留、派生和省略，Bridge statistics/capabilities/validation 不进入来源文档。A/B 重复投影的 Interchange SHA-256 分别稳定为 `3d7e75943f11786d50cdeb22a6adb69e5126ac8410a568810cd67670c3283105` 和 `1ab7cbebbb0d7b1f6a5c0f93d19f8bd606338c903e529d67db90e798f0dcb5e6`；正式 Schema、Pydantic、Adapter 与逐字段三方核对全部通过，A 输出 Canonical v2.4（46 任务、48 依赖、2 日历、5 里程碑），B 输出 v2.5（62 任务、65 依赖、2 日历、5 个约束、1 个 Deadline），来源保真均有效。新增可重复的 A/B 实际 API 门禁，两项目首次导入均返回 201、同信封重放均返回 200，来源/Canonical 双对象和双哈希逐字节验证通过，4 个测试对象、2 条 Snapshot 及级联记录和临时用户均清理并断言。补充 Projector 确定性/Schema/blocker/覆盖/Status Date 单元回归和正式 Schema 导出回归，Bridge 版本升至 `0.5.0-y0`，Contracts、Comparator、Projector、Package 门禁通过，连续打包 SHA-256 为 `75fb58362ec2daa7d2845443f95335f6c1276236487118fa911d7bbacf746806`。阶段更新为 `Y0 PASSED / Y1 READY`，生产结论保持 `PRODUCTION_NO_GO`。

- 完成 MPP Bridge M3/M4 退出门禁：项目方于 2026-08-24 明确批准将 `MPP_clean_v1` 的 A/B 按两份相互独立的受控真实项目进行本计划验收，同时保留输入包原有 `SYNTHETIC_BUSINESS_LIKE_TEST_DATA` 自述作为审计事实，不改写 provenance。A/B 分别以 46/62 个任务、48/65 条依赖完成 0 Unsupported、0 差异单命令 Roundtrip，Report/Diff Schema、来源不变和进程清理全部通过；完成判定 8 项和 M4 退出条件据此关闭，阶段状态更新为 `M3 PASSED / M4 PASSED / Y0 READY`。该决策只解除 Yuxi Y0 的启动顺序门禁，不代表 Yuxi 接入或生产门禁通过，生产结论继续为 `PRODUCTION_NO_GO`。

- 完成独立 MPP Bridge M1 Extract、M2 Build、M3 Comparator 和 M4 打包准备的工程实现：使用 Microsoft Project 16.0 从隔离工作副本输出严格 Snapshot、Manifest、Error、Semantic Hash，并从合格 Snapshot 创建全新 MPP，生成严格 Build Manifest、Build Error、Build Report 和 Identity Map；支持任意无继承多日历、七天工作周与多工作区间、默认/任务日历、一次性停工/补班例外、自动 Active/Inactive 任务、层级/WBS、汇总、里程碑、工期、FS/SS/FF/SF、Lag、MSO/SNET/FNET/FNLT 和 Deadline。MSP-006、水泵站、C08 三文件正式矩阵全部完成 `Extract → Build → 保存/关闭/重开/重算 → Extract`，分别以 5/21/21 个任务和 2/19/19 条依赖通过 Identity Map 核心语义对齐，反读均为 0 Unsupported、可 roundtrip，来源 MPP SHA-256 不变。派生 Snapshot 另验证三日历、任务日历、日历例外、Inactive 及四类约束保存重开；blocker、30 秒主动超时和超过 259 字符的 Project 路径均在启动或完成 COM 前 fail-closed，目标/staging 与 owned `WINPROJ` 无残留。新增字段级 Comparator、机器 Diff、人工摘要、严格 Roundtrip Report 与单命令闭环；Task ID/Unique ID 重分配可解释，日期、工期、日历、约束和依赖 Lag 漂移逐字段阻断，unsupported 不会误报 PASS。MSP-006 单命令 Roundtrip 及水泵站/C08 矩阵 Comparator 均为 0 差异 PASS。新增严格版本发现、完整错误恢复说明及确定性 ZIP 打包，最新源码连续构建的 ZIP SHA-256 均为 `78ed0e03b5a1245ebd5c7d7d42a1605a1339eba87e4ff471a95092b33ab871f0`。首个独立真实项目“北横泾总进度计划”已完成受控 Extract，1 个日历、90 个任务、90 条依赖通过 Schema、语义哈希、对象引用、来源字节不变和进程清理验证；该文件因 1 个手动任务及“9 个资源、0 个 Assignment”返回 2 个 blocker，按 v1 冻结范围 fail-closed，未执行 Build、不计入真实项目往返 PASS。新增明确标为 `SYNTHETIC_BUSINESS_LIKE_TEST_DATA` 的 `MPP_clean_v1` 技术矩阵：A/B 分别以 46/62 个任务和 48/65 条依赖完成 0 差异 Roundtrip；A 首轮暴露 5 个汇总任务日历未写回 blocker，已修正 Build 对汇总任务的任务日历写回并补充集成夹具，修复后 A/B 的 Report/Diff Schema、来源不变和进程清理全部通过。阶段状态为 `M2 PASSED / M3 IN PROGRESS`；合成技术样例不替代真实业务签字，两个独立真实项目的往返与人工复核、M4 最终批准和 Yuxi 接入尚未完成，生产结论保持 `PRODUCTION_NO_GO`。

- 完成复杂排期实施计划阶段 9 的工程切片：新增 Canonical v2.8 与 CPM Profile v15，inactive 任务保留来源日期并输出 `calculation_status=excluded_inactive`，但不进入 CPM、关键路径、汇总滚动、资源分析或工期优化；依赖触及 inactive 时不静默跨接，以稳定 Issue/Capability 阻断并进入 Dependency Workbench。Workbench 可把 `active → inactive → active` 的多条来源边归为一个 Issue，Candidate 同时移除全部已展示来源边并加入用户明确选择的 active 叶子关系；汇总任务依赖继续使用同一显式叶子替代机制。正式 `microsoft_project_interchange_v1.1` Adapter 升级到 1.6，仅在来源真实提供 `active: false` 时输出 v2.8。新增 Microsoft Project 16.0 独立 inactive/summary dependency Oracle，两次保存重开观测一致；观测确认 Project 会计算穿透 inactive，而 Yuxi 采用“不猜测跨接”的安全差异。工期优化任务选项与后端门禁保持一致，不再展示已有进度事实的任务；Dependency Workbench 在多条来源关系参数不一致时不再沿用第一条关系，而是要求重新明确类型和 Lag。同步完善排期用户及测试手册，增加一页式操作清单、自动化测试命令和进度任务、多来源依赖、inactive/汇总及甘特空值对齐专项 UAT；新增测试专用 Canonical 导出命令，可将 C01–C06、C08 转换为页面可上传 JSON，并明确跳过预期非法的 C07，且禁止向冻结测试包内部写入输出。Package 完整性、Schedule 单元测试 `259 passed`、Schedule API `18 passed`、v2.8 Oracle、两轮 C01-C08 及 16 个 JSON SHA-256 零差异、本次改动 Ruff、前端 Node 测试、ESLint、Web 与文档构建均通过。阶段状态更新为 `PHASE_9_ENGINEERING_DONE / PRODUCTION_GATES_PENDING`；两个独立真实业务项目、Worker E2E、生产规模性能/恢复、浏览器业务验收及技术和业务负责人批准仍未完成，生产结论保持 `PRODUCTION_NO_GO`。

- 完成复杂排期实施计划阶段 8：补齐 v2.3-v2.7 正式 `/snapshots → Audit → Optimization → Candidate` 产品链路，新增正式 `microsoft_project_interchange_v1.1` 来源协议并将 Adapter 升级到 1.5，旧 `microsoft_project_interchange_mock_v1.1` 仅保留兼容回归；正式 Adapter 不合成来源未提供的 Baseline、进度、Assignment 或费率。Delivery 统一只允许 `dependency_normalization + canonical_schedule_v2.2` 使用内置受控 Canonical 副本应用器，其他 Candidate/Canonical 组合返回 `DELIVERY_ADAPTER_UNAVAILABLE`，正式 Microsoft Project Adapter 不写回 MPP。前端补充“生产门禁尚未解除”和接入/来源审查/CPM 三状态，修正预检错误对象引用展示、方案对比切换器布尔值类型警告，并实测候选审阅、甘特和 Delivery 无白屏及新增控制台异常。冻结 Suite 连续两次严格 8/8，16 个输出文件 SHA-256 零差异，Package 完整性、240 项 Schedule 单元测试、17 项 Schedule API 集成、Ruff、9 个前端 Node 测试、ESLint、Web 与文档构建通过；Schedule Agent E2E 因本机 Worker 未消费 `pending` Run 记录为环境门禁未完成。阶段状态更新为 `PHASE_8_DONE / PHASE_9_PENDING`，Suite/产品入口闭环不等于生产可用，生产结论保持 `PRODUCTION_NO_GO`。

- 完成复杂排期实施计划阶段 7：扩展阶段 0 的唯一共享原始输入预检器，统一聚合任务、日历、依赖、资源和 Assignment 重复 ID，项目/任务/日历继承/Assignment 引用，任务父子环、日历继承环、依赖环及非零工期里程碑，并按错误码和对象引用稳定排序；v2.2 的 opaque Assignment 语义保持不变。`POST /api/schedule/snapshots` 在 Canonical 构造和任何持久化、Audit、CPM、Candidate 行为前返回结构化 `422 SCHEDULE_PREFLIGHT_FAILED`，字段类型错误继续由契约以 JSON Pointer 返回 `SCHEDULE_CONTRACT_INVALID`。C07 精确 6 个冻结 Issue 且 Engine 零调用，真实 HTTP/PostgreSQL 回归确认失败请求不产生 Snapshot/Candidate；237 项 Schedule 单元测试、14 项 Schedule API 集成、C01-C08 严格 8/8、Ruff、ESLint、前端和文档生产构建均通过。阶段状态更新为 `PHASE_7_DONE / PHASE_8_PENDING`，生产结论保持 `PRODUCTION_NO_GO`。

- 完成复杂排期实施计划阶段 6：新增 Canonical v2.7 强类型 Resource/Assignment 契约，冻结 WORK/EQUIPMENT、units、max units 和标准小时费率，并校验 Assignment 唯一 ID、任务/资源引用、正数 units 及仅活动任务分配。新增只读 `resource-analysis-v1` Profile，在 CPM 日期计算完成后按重叠区间检测 `RESOURCE_OVERALLOCATION`，使用十进制定点公式计算 Assignment 工作分钟和成本数值；资源分析不会移动任务，也不实现自动资源均衡或成本优化。Audit 可基于来源计划日期生成资源 Finding，Candidate API 和审阅界面展示资源、任务、重叠区间、容量及成本明细；资源 Capability 不再因 Assignment 存在而误报均衡可用。C06 冻结 expected、v2.7 `/snapshots → Audit → Candidate` 集成和“资源分析前后 CPM 日期一致”回归均通过。阶段状态更新为 `PHASE_6_DONE / PHASE_7_PENDING`，C01-C08 当前全部严格通过，但生产结论保持 `PRODUCTION_NO_GO`。

- 完成复杂排期实施计划阶段 5B：新增 CPM Profile v14，Canonical v2.6 含 IN_PROGRESS 时固定 actual start，并从 `max(status date, 网络要求边界)` 的下一工作时刻安排显式 remaining duration，以预测完成约束后续任务；反向浮时按剩余工作而非原始全工期计算，已完成任务不再被标记为仍有关键性。脱序 actual start 不被覆盖，统一产生 `ACTUAL_START_NETWORK_CONFLICT` 证据。v13 保持 completed + not-started 的冻结语义，Goal Optimizer 新增 `AUTHORIZED_TASK_IN_PROGRESS` 门禁，Candidate 页面展示进行中状态、剩余工期和剩余工作起点。完整 C05 冻结 expected 与 v2.6 `/snapshots → Audit → Candidate` 真实集成均通过。新增 Microsoft Project 16.0 状态日期独立 Oracle，真实 COM 捕获确认 `UpdateProject` 动作码 2 会保留 40% 完成和 1440 分钟剩余工期，Resume 为 2026-09-17 08:00，进行中任务与后续任务分别完成于 2026-09-21、2026-09-24；Oracle 门禁会检测 status date、动作码、actual、percent、remaining duration 与 Resume 漂移。阶段状态更新为 `PHASE_5B_DONE / PHASE_6_PENDING`，生产结论保持 `PRODUCTION_NO_GO`。

- 完成复杂排期实施计划阶段 5A：新增 Canonical v2.6 与 CPM Profile v13，要求显式 status date，并校验 NOT_STARTED、IN_PROGRESS、COMPLETED 与 actual start/finish、remaining duration、Baseline 0 的一致性；v13 固定 COMPLETED 实际日期，以实际完成约束后续 NOT_STARTED，并按任务有效日历输出 Baseline 0 开始/完成工作分钟偏差。Goal Optimizer 明确排除 completed，Candidate 任务表新增实际事实和 Baseline 展示，v2.6 `/snapshots → Audit → 自动重算 Candidate` 真实集成通过。C05 的 completed + not-started 子集通过，完整 C05 仅以 `IN_PROGRESS_UNSUPPORTED` 保持关闭；当前正式 Adapter 没有可追溯进度字段，因此不合成 v2.6。阶段状态为 `PHASE_5A_DONE / PHASE_5B_PENDING`，生产结论保持 `PRODUCTION_NO_GO`。

- 完成复杂排期实施计划阶段 4 的工程实现：新增 Canonical v2.5 与 CPM Profile v12，支持 MSO 固定开始、FNLT 完成上界、任务 Deadline 和项目 required finish；MSO 与网络冲突时保留固定日期并输出证据，FNLT/Deadline/required finish 只报告工作分钟偏差，不截短工期或静默移动任务。C04 已严格匹配冻结 expected，v2.5 Snapshot、Audit、自动重算 Candidate、Delivery 溯源与前端人类可读冲突提示已接通。Microsoft Project Interchange Adapter 升级到 1.4，可追溯的约束码 2/7、Deadline 和 required finish 输出 v2.5。已在可见交互会话中捕获 Microsoft Project 16.0 独立 MSO/FNLT observation 并通过外部观测结构校验；结果显示 Project 会让 FNLT 优先于 FS 网络并提前任务，而 Yuxi 当前保留网络日期、只报告违反，另有三项浮时口径差异。真实 observation 和差异回归已固化，差异按 Yuxi 网络优先安全规则接受并持续门禁，不宣称与 Project 完全一致，生产结论保持 `PRODUCTION_NO_GO`。

- 完成复杂排期实施计划阶段 3B：Canonical v2.4 增加 `SUCCESSOR_TASK_CALENDAR` 冻结值，新增 CPM Profile v11，以任务有效日历计算工期、以后续任务有效日历计算正负 Lag，并让 Forward、Reverse、总/自由浮时和关键标识共享解析后的多日历事实；支持父日历先解析、子日历工作周与例外覆盖，未知父日历、继承环、无工作时间和冲突例外继续明确阻断。C02 已与冻结 expected 严格匹配，跨日历 FS/SS/FF/SF、负 Lag、继承覆盖、Audit Capability、v2.4 `/snapshots` 和 Candidate v11 溯源均增加回归。Microsoft Project Interchange Adapter 升级到 1.3，多/任务日历来源输出 v2.4 与 successor-calendar policy。新增 1000 任务、5000 依赖、双日历基准；Windows 11 / Python 3.13.14 本机 20 次引擎运行 p50 33.65 ms、p95 34.70 ms、最大 52.80 ms，并采用 p95 ≤ 50 ms 门禁。

- 完成复杂排期实施计划阶段 3A：新增 Canonical v2.4 结构化单项目日历例外契约、JSON Schema 和版本路由，支持停工日、连续停工与补班；抽离 `EffectiveWorkCalendar`，使正向、反向、有符号 Lag、浮时和关键标识共享同一例外日历事实。新增 CPM Profile v10，C02 单日历子集日期通过，冲突范围、例外时区不一致、多/任务日历和日历继承继续明确阻断，完整 C02 仍仅因多日历为 `UNSUPPORTED`。Microsoft Project Interchange Adapter 升级到 1.2，有结构化例外时输出 v2.4；Import、Audit、Candidate、双对象/双哈希持久化与正式文档同步接入。

- 完成复杂排期实施计划阶段 0/1 与阶段 2：新增只读 Suite Runner、共享聚合预检和 `suite_issue_profile_v1`，C01、C03、C07、C08 已与冻结 expected 严格匹配，其余案例以准确 `UNSUPPORTED` 保持关闭；新增 Canonical v2.3（独立 milestone 与生产 `boundary_role`）及 discriminator 版本路由，Audit、重算、目标优化、Candidate、Delivery 和案例预检统一分发。新增 milestone CPM Profile v8，保证里程碑零工期、正反向日期相等并参与四类依赖、浮时、关键标识和汇总滚动；新增 signed-lag CPM Profile v9，在单一有效项目日历下支持 FS/SS/FF/SF 负 Lag，并将有符号工作分钟统一用于正向、反向、浮时和 locked 冲突证据。Microsoft Project Interchange Adapter 升级到 1.1，含里程碑来源输出 v2.3、无里程碑来源继续输出 v2.2。Candidate 新增 Canonical/Adapter/Engine 版本溯源列与兼容迁移，比较甘特增加里程碑菱形和缺失日期过滤；新增 v2.3 JSON Schema、回归测试与正式文档。已通过隐藏 COM 会话采集 Microsoft Project 16.0 四类负 Lag 独立 observation，Yuxi v9 日期、浮时和关键标识与观测一致，用户已确认 expected，黄金门禁为 `PASSED`。多/任务日历例外、硬约束、实际进度和资源成本继续保持阻断。

- 工期优化/重算结果展示闭环：方案审阅抽屉新增“方案对比”甘特，对比口径按方案类型区分——工期目标优化对比引擎基线（未优化重算）与优化后，排期重算对比来源计划与重算结果，变化任务深色描边、完成时刻竖线标记；修正结果摘要“来源完成”标签在目标优化下误标的问题（goal 的 `finish_before` 是引擎基线完成）、甘特布局字段名不一致导致打开方案审阅时白屏的问题，以及任务条重复计算左侧任务列宽造成的日期错位。工期目标优化表单现支持逐条添加 1–10 个授权活动任务并分别填写压缩后工期，前端校验任务唯一、工期缩短且与 locked tasks 互斥，后端补充必须组合两个授权任务才能达标的回归测试。主页面“计划内容”卡片支持“来源计划/方案对比”切换，对比模式表格与甘特共用同一行序（按来源任务顺序），列按方案类型显示基线/来源与优化/重算日期；新增只读接口 `GET /api/schedule/snapshots/{id}/candidates` 与“已生成方案”卡片，按类型、有效状态、审阅态度和完成时间变化列出当前快照候选，刷新后可重新打开审阅。工期目标优化审阅抽屉扩展到 `min(1480px, 98vw)`；任务结果表按真实 `duration_changes` 用绿色标识直接优化任务，并用蓝色区分仅受网络联动影响的日期变化任务，文字标签与浅色行背景同步呈现且兼容暗色主题。新增 `ScheduleComparisonGantt.vue` 复用甘特几何工具，后端补充候选列表服务方法与单测，相关 Schedule 单元测试、ESLint 与前端生产构建均通过。

- 排期审查页面新增“计划内容”卡片：任务表展示 WBS、名称、类型（汇总/活动/里程碑）、工期、计划开始/完成，任务按层级缩进且汇总行浅色背景加粗；下方配套轻量甘特图按来源计划日期绘制日历跨度条（周末灰显、汇总浅色条、里程碑菱形标记、长排期自适应列宽并横向滚动），标题栏显示项目计划起止；数据直接取自快照正文，无后端与接口改动。新增纯函数甘特几何工具 `scheduleGantt.js`（时间窗、条定位、周末与自适应列宽，按日期字符串归一化避免浏览器时区影响）及 node:assert 单元测试；ESLint 与前端生产构建通过，用户手册同步补充“计划内容”小节。

- 冻结排期 Lag 日历策略为统一项目日历工作分钟：Canonical v2.2 契约与 JSON Schema 新增 `UNIFIED_PROJECT_CALENDAR_WORKING_MINUTES` 取值，MPP 导入适配器在语义与依赖级别写入该冻结值；来源 Audit 对非零正负 Lag 按统一项目日历工作分钟执行确定性日期合规检查（FS/SS/FF/SF 锚点按有符号 Lag 位移后比较，跳过午休与周末，复用引擎工作分钟算法），新增 `LAG_DATE_VIOLATION` blocker 规则，`LAG_CALENDAR_POLICY_UNSPECIFIED` 只对策略未冻结或日历口径不支持的未检查关系上报。水泵站 Import 案例为 19 条 checked、0 条 skipped 且无新增违规；S2 候选新增依赖继承基础快照冻结策略，避免候选回退为“未冻结”。策略未冻结的旧快照行为保持不变。补充契约接受、正负 Lag 检查、违规检测与日历口径阻断单元测试，同步导入适配器与 Agent E2E 断言。

- 新增排期工期目标优化闭环：支持 `MINIMIZE_PROJECT_FINISH` 与 `MEET_TARGET_FINISH`，请求绑定 Snapshot 内容哈希并要求显式授权，只允许将 1–10 个自动活动任务缩短到业务已确认的工期，依赖、Lag、日历、里程碑和任务模式保持不变，locked tasks 日期冻结。新增确定性 `yuxi-authorized-duration-goal-optimizer-v1`，复用 v7 CPM 比较基线和全部有限授权组合，并按目标分别执行稳定的最早完工或最少压缩排序；目标不可达返回 `TARGET_FINISH_UNACHIEVABLE` 和最佳可达策略，不持久化伪成功 Candidate。成功结果持久化授权记录、工期 Patch、Candidate Audit、比较指标和完整日期/浮时/关键标识，Source 保持不变且 Delivery 生成后立即可读；排期页面新增授权弹窗、阻断展示和目标优化 Candidate Drawer。新增安全投影工具 `get_schedule_goal_optimization_context` 和 Goal Agent 模式，只梳理目标与授权并生成未发送草稿，最终仍需回排期页明确授权提交。补充 Microsoft Project 双次确认的分叉汇合工程案例、幂等/冲突/权限/安全投影/真实 API/Agent E2E/前端回归和浏览器成功、不可达、Delivery、Agent 草稿验收；159 项相关单元测试、9 项排期 API 集成测试、1 项 Agent E2E、前端测试、Lint 和生产构建均通过。当前仍不包含资源或成本优化，生产门禁保持 `PRODUCTION_NO_GO`。
- 新增整份计划 AI 审查入口：排期页面“审查问题”标题栏可选择同时具备三个 Schedule 工具的智能体，并将当前 Snapshot ID、内容哈希、外部项目和版本绑定到新对话；对话持续显示排期身份，提供三类仅预填不自动发送的建议问题，切换线程时清除旧排期上下文。新增只读工具 `get_schedule_review_context`，使用运行时 `uid` 校验归属，只投影 Audit、Capability、Issue Summary 及最多 50 个问题摘要和证据链接，不向模型暴露完整来源 JSON、Notes、unknown/ignored/unsupported 字段或 Issue 原始 evidence；Agent 行为边界同步要求 Capability 阻断优先、同级顺序标为建议、无版本化证据时不判断工程业务合理性，具体 Issue 可返回排期页面打开证据。单 Issue“Agent 解释”继续兼容只启用原两个工具的智能体，整份审查独立要求新工具；补充投影权限、工具元数据、Agent 资格、前端安全路由和整份审查 E2E 回归。同步落实 Candidate 生成即交付的既定口径：Delivery 在 `not_reviewed`、`accepted`、`rejected` 三种态度下均可读取，用户态度不再充当交付或应用门禁，技术状态、版本状态和外部业务审批继续独立判断。
- 新增排期外部 JSON 导入链路：定义带严格身份字段的 `ScheduleImportEnvelope` 和允许来源扩展字段的 `microsoft_project_interchange_mock_v1.1` 边界契约，缺少核心字段、已定义字段类型错误、未知引用和父子层级环仍会拒绝；新增按 `schema_version` 路由的 Adapter Registry，以及纯 Python、跨平台的水泵站 Microsoft Project interchange 适配器，在不读取 MPP、不依赖 Windows 的前提下生成严格 `canonical_schedule_v2.2`。来源 JSON 数据语义与未知字段被保留，来源自报 Statistics、Capabilities 和 Validation 不参与内部结论，Yuxi 重新计算 21 个任务、19 条依赖、开放起止点和 Capability；来源缺失的 Baseline、Source Calculation、项目当前日期及资源分配通过规范化报告显式标记为未参与审查或计算，4 个零工期里程碑使 CPM 明确返回 `MILESTONE_UNSUPPORTED`。新增 `POST /api/schedule/imports`，在写入前完成适配和 Audit，分别以私有对象保存来源文档与严格 Canonical，并记录来源哈希、Canonical 哈希、来源版本、适配器版本和规范化报告；首次请求返回 201，内容一致的幂等重放返回 200，同一 `request_id` 的来源变化或与原 `/snapshots` 入口交叉复用返回 409。现有 `/api/schedule/snapshots` 行为保持不变，普通用户响应和读取接口不暴露外部来源对象正文。排期审查页面新增“外部接入、来源审查、CPM 重算”三阶段状态、来源格式、适配器版本、规范化计数和 unsupported 原因展示，只显示字段数量及影响对象数量，不显示来源未知字段值；Agent 入口草稿与 Schedule 工具说明同步禁止把来源 Validation 或 ignored/unsupported 字段描述成已参与 Yuxi 审查或计算。新增正式外部 JSON 导入指南，并同步用户手册、维护手册和文档站导航；代码审查后进一步明确来源审查不等于 MPP 提取可信、来源文档与 Canonical 的区别、三类哈希、Capability 与规范化报告的判定优先级，以及真实集成测试的清理风险。排期审查页面现提供统一“导入排期”入口，自动识别版本化来源 JSON 与严格 Canonical、预填并确认外部身份、按格式调用 `/imports` 或 `/snapshots`，成功后自动定位新快照，并对 422、409 和可重试存储失败提供明确反馈；同一 `request_id` 改变任一外部身份现会返回冲突，提取证据为 false 时来源审查与报告统一返回 `SOURCE_FIDELITY_INVALID`，无里程碑时不再误报 `MILESTONE_UNSUPPORTED`，前端错误日志不再输出请求正文或认证头。既有真实 HTTP/PostgreSQL/MinIO 集成测试、Microsoft Project 黄金门禁、Import→Agent E2E 和水泵站浏览器验收此前均已通过；本轮新增回归后，139 项 Schedule 单元测试、前端导入逻辑测试、定向 ESLint、前端与文档生产构建及来源/Canonical 页面预览均通过。
- 新增排期审查与 CPM 重算用户及测试手册：面向业务用户、计划人员、测试人员和试点组织者，说明来源快照、能力边界、审查问题、重算方案、最早/最晚日期、总/自由浮时、关键任务、接受/拒绝及 Delivery 的正确使用方式；提供 15 项编号化 UAT 用例、范围外阻断与 Source 不可变检查、反馈优先级、首轮试点指标、问题记录和测试完成报告模板，并明确“接受不等于应用”及当前 `PRODUCTION_NO_GO` 边界。文档已加入站点导航，原维护手册同步增加用户入口。
- 新增排期 S3/S4 受限正向 CPM：Profile `yuxi-forward-unified-calendar-fs-ss-ff-sf-positive-lag-snet-fnet-manual-v5` 按单一无继承项目日历计算 FS/SS/FF/SF 零/正 Lag、ASAP/SNET/FNET 与手工活动任务的最早开始/完成；正 Lag 按工作分钟推进，FF/SF 从关系完成下界按后续任务工期反推开始，SNET/FNET 与依赖取最严格下界。普通手工任务以来源日期为开始下界，有依赖冲突时按 Project 基线调整；重算请求可显式传入 `locked_task_ids`，locked 日期保持不动，依赖要求移动时生成 `invalid` Candidate 并禁止 Delivery。日期只保存在 Candidate `engine_result`，Source 与来源计算字段不变。负 Lag、多日历/例外、汇总依赖、非法约束组合和实际进度返回结构化 `blocked`，不做近似计算。用户已确认五套 Microsoft Project observation，expected 已回填且五套门禁均为 `PASSED`。同步修正 M4.5 Benchmark 对合法 SNET 模板的 Capability 声明。第一阶段来源 Audit 对非零 Lag 日期合规仍保持 skipped，不与 CPM 计算能力混淆；当前不声明生产或跨项目通用性。
  新增 v6 汇总任务日期滚动 Profile：显式保留 v5 历史语义，生产正向重算在活动任务计算完成后按层级从深到浅滚动汇总任务开始/完成，并在 `engine_result.task_dates`、Candidate Diff 和 Delivery 中输出活动与汇总任务日期；来源 Snapshot、来源汇总日期和 `source_calculation` 均不改写。新增 MSP-006 v2 合法 canonical 适配、第六套 Microsoft Project 黄金门禁、两层嵌套滚动断言和真实 HTTP 主链路回归；第六门禁为 `PASSED`。
  新增 v7 反向日期、浮时和关键标识 Profile：完整继承 v6 的统一日历、FS/SS/FF/SF 零/正 Lag、SNET/FNET、手工/locked 和嵌套汇总能力，在活动任务正向计算后从候选项目完成日期反向计算最晚开始/完成，以项目日历工作分钟输出总浮时、自由浮时，并按 `total_slack_minutes <= 0` 标记关键任务；汇总结果继续按直接子任务自底向上投影，Source 和来源计算字段保持不变。新增可区分 total/free slack 的五任务分叉汇合 Oracle、Microsoft Project 16.0 两次独立 COM 复采集证据、第七套黄金门禁和 v6 全能力正向继承回归；排期审查页面新增按任务展示来源/最早/最晚日期、总/自由浮时和关键标识的结构化审阅表，关键活动任务单独计数，原始日期 Patch 默认折叠为技术明细。七套门禁、121 项 Schedule 单元测试与 7 项真实 HTTP/PostgreSQL/MinIO 集成测试均通过，前端已用隔离的受支持快照完成浏览器主链路验收并确认 Source 内容哈希不变。该结果仍不代表 G2、生产发布或跨项目通用性通过。
- 新增排期审查第一阶段闭环：支持 `canonical_schedule_v2.2` 严格契约、JSON Schema 漂移检查、脱敏黄金样例、不可变 Snapshot、PostgreSQL/私有 MinIO 持久化、并发幂等、60 秒执行租约与失败/取消恢复；Yuxi 独立计算 Statistics、Capability 和 16 类确定性 Issue，当前黄金样例稳定得到 68 条零 Lag 已检查、22 条非零 Lag 未检查、2 个开放起点、15 个开放终点和 4 个汇总任务依赖。新增 `/schedule` 只读页面、证据与直接上下游 Drawer、两个只读 Agent 工具以及仅预填不自动发送的 Agent 入口；修复 Agent 解释调用中 `ToolRuntime` 被自定义参数模型丢弃、配置页序列化内部运行时参数导致 Schedule 工具无法显示、相同请求的并发等待早于 60 秒租约结束，以及页面无法继续读取首批以外 Snapshot/Issue 的问题，分类筛选不再依赖当前页数据；补充真实工具入口、工具元数据和并发等待回归测试，并修正开发端口说明。第二阶段新增 S1 汇总依赖确认工作台：后端返回上下游局部任务树、入口/出口叶子候选和直接依赖，持久化可覆盖草稿及不可变确认记录；前端支持选择替代叶子关系、新增里程碑或暂不处理，并明确区分保存与确认。新增 S2 单条汇总依赖规范化闭环：已确认关系可幂等生成唯一 `schedule_candidate_draft_v0`，在 Source 副本删除原汇总依赖并添加授权的叶子依赖，执行引用、重复关系、环路和第一阶段 Audit 校验，先重投影候选 Statistics/Capability 再展示 Requested/Effective Patch 与无统计漂移的 Issue Diff，追加接受/拒绝历史，并在接受后提供版本状态受控的 Delivery Package；受控 E2E 来源适配器严格校验 accepted/valid/current、Candidate Kind、基础 Hash 和 Patch 前置值，只生成独立 Source 副本并同步重算来源 Statistics/Capability/Validation，再通过现有 Snapshot 接口完成 PostgreSQL、MinIO 与 Audit 回流。新增只读回流验收证据接口与页面 16 项检查表，自动聚合基础/回流对象 Hash 与数据库记录一致性、request/external snapshot/external revision/source snapshot 版本身份独立性、Patch、新 Snapshot Audit、新增 Blocker 和旧 Candidate 过期状态，明确区分待回流、已通过和未通过，避免把同项目无关的新 Snapshot 误认成本次 Delivery 回流。真实业务首条链路已验证原 Source/Hash 不变、目标汇总依赖消失、叶子依赖参数一致、无统计漂移和新增 Blocker。为避免首条案例成为隐性模板，S1/S2 生产实现不绑定项目、任务或依赖 ID，并补充前置/后置/双端汇总、多入口/多出口笛卡尔展开、FS/SS/FF/SF、Lag 值投影、重复选择、25,000 条依赖容量、完整基础 Issue 集和最后一条汇总依赖 Validation 清理矩阵；新增隐私安全的案例预检入口，只输出契约位置、内容指纹和结构差异，显式区分真实、脱敏与合成案例，并分别返回工程协议与真实业务迁移结论。新增从零构造的确定性合成案例 S（9 个任务、4 层任务树、FS/SS/FF/SF、前置端汇总依赖），已通过 Candidate/Decision/Delivery/Source 回流和 16 项验收；该案例只增加工程迁移证据，机器可读标记禁止其替代第二个独立真实案例 B。第二个独立真实排期案例移至 G2/生产迁移验收，不再阻断 S3 工程开发。S1/S2 均不原地修改 Source、不运行 CPM。脱敏回归同时与私有 Source 文件解耦，私有业务 JSON 路径加入 Git 忽略规则。CandidateSnapshot 暂以 `schedule_candidate_draft_v0` 作为内部实验契约，不冻结长期字段。同步新增排期审查操作与维护手册。
...
- 新增新业务智能体开发指南：明确数据库 Agent 与 Python Agent Backend 的边界，提供从业务验收标准、配置型 Agent、Skill / Tool / MCP / 知识库 / 子智能体选型，到自定义 Backend 自动发现、Context 扩展、测试调试和交付检查的端到端流程；补充可直接落地的合同审查 Backend 教程，覆盖 Backend 单例边界、完整 Context / Prompt / Graph、中间件取舍、摘要配置、私有工具、自定义 State 与前端投影边界、API 创建、装配单测、原生 StateGraph 升级条件及常见故障；同步修正智能体配置文档中的 Backend 目录说明，明确当前自动发现仅扫描 `backend/package/yuxi/agents/buildin` 的直接子目录。
...
- 完成 K-AI 第一阶段可视化品牌改造：默认品牌配置调整为“量程科技 · K-AI”，产品定位更新为建筑行业智能知识库与智能体平台；浏览器标题、favicon、登录页、首页、主侧边栏、设置与用户菜单、CLI 授权页及知识库类型标签统一使用新品牌展示，并移除上游 GitHub Star、仓库和帮助文档等产品界面入口。内部 `yuxi` 包名、环境变量、接口协议、缓存键、Docker 与数据库结构保持不变；已接入量程科技正式 Logo 并同步生成应用与文档站 favicon。修复宿主机从仓库根目录启动 API 时品牌配置使用相对工作目录导致 `/api/system/info` 返回空对象、首页 Logo 破图的问题；默认品牌文件改为基于后端源码目录定位，前端组织信息补充正式 Logo 回退。官网和帮助地址仍待提供。
