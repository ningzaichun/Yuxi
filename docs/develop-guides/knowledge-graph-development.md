...
该配置不在“知识图谱”页签中，而是在“检索测试”页签的检索配置面板中。使用管理员账号打开 Milvus 知识库后：

1. 进入知识库详情，点击第二个页签“检索测试”。
2. 在页面右侧找到“检索配置”。浏览器宽度小于 1024 像素时，该面板会移动到检索测试区域下方。
3. 在“检索配置”面板内部向下滚动，找到“启用图检索”下拉框。它是“启用/关闭”下拉框，不是开关按钮。
4. 选择“启用”。依赖图检索的参数会在下方显示。
5. 初次使用保留以下默认参数：
...
6. 点击面板右上角“保存”。刷新页面后，“启用图检索”仍显示为“启用”，说明配置已经持久化。

如果找不到该字段，先确认当前知识库类型为 Milvus、当前账号具有管理员权限，并且滚动的是右侧“检索配置”面板而不是整个页面。配置项由
`GET /api/knowledge/databases/{kb_id}/query-params` 动态返回；接口响应的 `params.options` 中存在
`key=use_graph_retrieval` 时，前端应显示该字段。
...
- `pending_chunks` 为 0。
- 实体数和关系数均大于 0。
- 可以搜索到项目、建筑、组织或人员等核心实体。

然后使用同一个跨文档问题进行两轮检索：

1. 在“检索测试”右侧关闭图检索并保存，执行问题，记录基础召回结果。
2. 启用图检索并保存，重新执行同一个问题。
3. 点击检索输入区右下角的 `{}` 按钮切换到原始数据显示。
4. 检查返回 Chunk 的顶层字段。`fusion_sources` 包含 `"graph"` 是图检索参与排名融合的直接证据；图召回 Chunk 还可能包含 `graph_score`：
...
格式化结果卡片当前不会展示 `fusion_sources` 和 `graph_score`，因此验证时必须查看原始数据。仅凭回答内容发生变化，不能证明图检索已经执行。

如果启用后仍只有基础结果：

1. 检查原始结果是否完全没有 `fusion_sources`，或其中是否没有 `"graph"`。
2. 检查 API 日志中是否出现 `Graph retrieval failed`。
3. 检查 Milvus 中的图实体、图三元组 Collection，以及 Neo4j 中该知识库的实体和关系。
4. 确认知识库配置中使用的 Embedding 模型仍然可用。

图检索异常时，当前检索链路会记录错误并保留基础检索结果，所以“仍然返回了文档”不等于图检索正常。
