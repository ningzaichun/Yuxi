...
项目也提供三层拆分方式：`deploy/split/docker-compose.infra.yml` 在基础设施服务器编排 PostgreSQL、Redis、MinIO、Milvus/Etcd 与 Neo4j；`deploy/split/docker-compose.app.yml` 可在独立应用服务器运行生产态 API、Worker、Web 和 Sandbox Provisioner；开发机通过 `scripts/split-deploy/Start-HostService.ps1` 直接运行相同程序进行源码调试。三种角色共享同一业务代码与运行链路，但生命周期、环境地址和进程管理相互独立。
...
- 根 `docker-compose.yml` 是一体化开发环境的事实来源；使用拆分方式时，基础设施以 `deploy/split/docker-compose.infra.yml` 为准，独立应用服务器以 `deploy/split/docker-compose.app.yml` 为准，本机调试入口以 `scripts/split-deploy/Start-HostService.ps1` 为准。不要混用不同角色的进程管理和连接地址。
