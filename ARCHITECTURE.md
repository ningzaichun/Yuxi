...
日常开发采用混合拓扑：PostgreSQL、Redis、MinIO、Milvus/Etcd 与 Neo4j 位于远程基础设施服务器；开发机通过 `scripts/split-deploy/Start-HostService.ps1` 在宿主机运行 API、Worker 和 Web；本机 Docker 只常驻 `sandbox-provisioner`，并按线程动态创建真正执行命令和处理文件的 Sandbox Runtime 容器。宿主机程序与沙盒共享根目录 `saves/threads`。

根 `docker-compose.yml` 保留完整本地 Compose 能力，日常开发只需从中启动 `sandbox-provisioner`。最终部署支持两种方式：

- 一体化生产部署：`docker-compose.prod.yml` 在一台服务器运行基础设施、API、Worker、Web 和 Sandbox Provisioner。
- 拆分生产部署：`deploy/split/docker-compose.infra.yml` 运行基础设施，`deploy/split/docker-compose.app.yml` 在另一台服务器运行 API、Worker、Web 和 Sandbox Provisioner。

三种运行形态共享同一业务代码与请求链路，但进程生命周期、环境地址和文件挂载不同，不应混用配置或进程管理命令。
...
5. Worker 执行 LangGraph 智能体；中间件按上下文挂载知识库工具、Skills、MCP、附件与沙盒能力。
6. 运行事件写入 Redis，最终状态和业务记录写入 Postgres；文件和产物落到 `saves`、MinIO 或沙盒用户数据目录。
7. 前端通过 SSE/轮询消费运行事件，渲染消息、工具调用、引用来源、产物卡片和文件预览。

## 架构不变量

- 本地开发以 `deploy/split/.env.host.dev.template`、`scripts/split-deploy/Start-HostService.ps1` 和根 `docker-compose.yml` 的 `sandbox-provisioner` 服务为事实来源；一体化生产以 `docker-compose.prod.yml` 为准；拆分生产分别以两个 `deploy/split` Compose 文件为准。
- `sandbox-provisioner` 是控制面，Sandbox Runtime 是它动态创建的执行容器；本地开发必须让二者与宿主机 API/Worker 看到同一个 `saves/threads`。
